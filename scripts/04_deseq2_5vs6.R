...
sig_mask           <- !is.na(df$padj) & df$padj < alpha & abs(df$log2FoldChange) > 1
fdr_pval_threshold <- if (any(!is.na(df$padj) & df$padj < alpha)) max(df$pvalue[!is.na(df$padj) & df$padj < alpha], na.rm = TRUE) else alpha
